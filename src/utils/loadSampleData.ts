/**
 * Utilidad para cargar datos de ejemplo en desarrollo
 * Se ejecuta automáticamente si no hay datos en localStorage
 */
...
export const loadSampleDataIfNeeded = (): void => {
  try {
    const existing = localStorage.getItem(STORAGE_KEY)
    if (!existing || existing === '[]' || existing === 'null' || existing.trim() === '') {
      console.log('📊 Cargando datos de ejemplo para desarrollo...')
      localStorage.setItem(STORAGE_KEY, JSON.stringify(sampleData))
      console.log('✅ Datos de ejemplo cargados:', sampleData.length, 'horas')
      console.log('💡 Recarga la página para ver los datos en el grid')
    } else {
      try {
        const parsed = JSON.parse(existing)
        if (!Array.isArray(parsed) || parsed.length === 0) {
          console.log('📊 Datos inválidos encontrados, cargando datos de ejemplo...')
          localStorage.setItem(STORAGE_KEY, JSON.stringify(sampleData))
          console.log('✅ Datos de ejemplo cargados:', sampleData.length, 'horas')
        }
      } catch {
        console.log('📊 Error al parsear datos, cargando datos de ejemplo...')
        localStorage.setItem(STORAGE_KEY, JSON.stringify(sampleData))
        console.log('✅ Datos de ejemplo cargados:', sampleData.length, 'horas')
      }
    }
